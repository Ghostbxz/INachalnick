...
public class SwaggerRequestExamplesOP : IOperationProcessor
{
    public bool Process(OperationProcessorContext context)
    {
        ...
        if (exampleAtt == null || exampleAtt.JsonExamples.GetEmptyIfNull().Count() <= 0) { return true; }
        var examples = new Dictionary<string, OpenApiExample>();
        foreach (var ex in exampleAtt.JsonExamples)
        {
            examples[$"example - {examples.Keys.Count + 1}"] = new OpenApiExample { Value = ex.FromJson<dynamic>() };
        }
